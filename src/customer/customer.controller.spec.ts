  ...
  describe('create', () => {
    ...
    describe('findMyCustomers', () => {
          ...
          data: [
            {
              ...
              c_email: 'my@customer.com',
            },
          ],
        };

        service.findCustomersByUserId = jest.fn().mockResolvedValue(mockResponse);

        const result = await controller.findMyCustomers(mockRequest);

        expect(service.findCustomersByUserId).toHaveBeenCalledWith(1);
        expect(result).toEqual(mockResponse);
      });
      ...
      it('should use userId from request', async () => {
        ...
        const mockResponse = { code: 1, status: true, message: 'Success', data: [] };

        service.findCustomersByUserId = jest.fn().mockResolvedValue(mockResponse);

        await controller.findMyCustomers(requestWithUserId as any);

        expect(service.findCustomersByUserId).toHaveBeenCalledWith(5);
      });
    });
    ...
    describe('findOne', () => {
      ...
      it('should convert string id to number', async () => {
        const mockResponse = { code: 0, status: false, message: 'ไม่พบข้อมูลลูกค้า', data: null };
        service.findOne = jest.fn().mockResolvedValue(mockResponse);
    ...
    describe('edge cases', () => {
      ...
      it('should handle non-numeric string id parameters', async () => {
        const mockResponse = { code: 0, status: false, message: 'ไม่พบข้อมูลลูกค้า', data: null };
        service.findOne = jest.fn().mockResolvedValue(mockResponse);
      ...
      it('should handle null user in findMyCustomers', async () => {
        const nullRequest = { user: null } as any;

        // ควรจะ throw error เมื่อพยายาม access userId จาก null
        expect(() => controller.findMyCustomers(nullRequest)).toThrow();
      });
    });
  });
});